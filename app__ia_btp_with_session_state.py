...
if 'back_dataframe' not in st.session_state:
    st.session_state["back_dataframe"] = pd.DataFrame()

# With magic:
#"data transformation", st.session_state["dataframe"]
...
if checkbox_val:
    ...
    if next_step == 'Fill missing values':  
        ...
        if st.session_state["back_dataframe"].empty:
            ...
            res = st.session_state["back_dataframe"]
        ...
        while i < int(genre):
            try:
                ...
                boolean_condition = res["Fournisseurs IA-BTP"] == name
                column_name = "E-MAIL"
                new_value = email

                res.loc[boolean_condition, column_name] = new_value
        ...
        st.session_state["filled_dataframe"] = res
        ...
        if st.session_state["filled_dataframe"] is None:
            
            if st.session_state["back_dataframe"].empy:
            
                reso = st.session_state["dataframe"]
            else:
                reso = st.session_state["back_dataframe"]
        else:
            reso = st.session_state["filled_dataframe"]
        ...
        if pdf_file is not None and pdf_file.name[-3:] == 'pdf':
            ...
            for i in range(0,len(L),2):
                boolean_condition = reso["Fournisseurs IA-BTP"] == L[i] #val_fournisseur
                reso.loc[boolean_condition, column_name] = L[i+1] # val_email 

            
            
            st.session_state["dataframe"] = reso

            st.session_state["filled_dataframe"] = reso
            
            st.session_state["back_dataframe"] = reso
